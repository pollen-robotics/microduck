...
fn recorded_policy_action(action: Option<[f32; duck_control::ACTION_LEN]>) -> Vec<f32> {
...
fn load_controller(policy_cfg: &params::ResolvedPolicy) -> Result<Option<Controller>, String> {
    ...
    let mut policy = Policy::load(&paths, DEFAULT_STANDING_THRESHOLD).map_err(|e| e.to_string())?;
...
fn build_controller(
    ...
    match load_controller(policy_cfg) {
        Ok(Some(controller)) => {
            tracing::warn!(mode = policy_cfg.mode.as_str(), walk = %policy_cfg.walk.display(), limp_fall, "policy loaded");
            Some(controller)
        }
        Ok(None) => {
        ...
        Err(error) => {
            tracing::error!(error = %error, "policy unavailable; holding the pose");
...
async fn control_loop<T: RobotIo>(
    ...
    // The candidate loads off this thread. A one-slot channel is enough: SIGHUP is a level and
    // a newer request after a worker starts will schedule one more load, never a backlog.
    let mut reload_worker: Option<mpsc::Receiver<Result<Option<Controller>, String>>> = None;
    ...
    while !state.shutdown.load(Ordering::Relaxed) {
        ...
        // A model reload is deliberately more conservative than a mode switch: do not begin it
        // while the policy is enabled. The candidate's ONNX sessions are built on another thread;
        // only the finished controller crosses into this loop, at this tick boundary.
        if !snapshot.enabled {
            if let Some(worker) = reload_worker.as_ref() {
                match worker.try_recv() {
                    Ok(Ok(candidate)) => {
                        controller = candidate;
                        state.policy_error.store(None);
                        reload_worker = None;
                        tracing::warn!("policy reload accepted while disabled");
                    }
                    Ok(Err(error)) => {
                        reload_worker = None;
                        tracing::error!(error = %error, "policy reload rejected; keeping current controller");
                    }
                    Err(mpsc::TryRecvError::Empty) => {}
                    Err(mpsc::TryRecvError::Disconnected) => {
                        reload_worker = None;
                        tracing::error!("policy reload worker stopped without a result");
                    }
                }
            }
            if may_start_reload(
                snapshot.enabled,
                reload_worker.is_some(),
                state.reload_requested.load(Ordering::Relaxed),
            ) && state.reload_requested.swap(false, Ordering::Relaxed)
            {
                let candidate_cfg = policy_cfg.clone();
                let (tx, rx) = mpsc::sync_channel(1);
                std::thread::spawn(move || {
                    let _ = tx.send(load_controller(&candidate_cfg));
                });
                reload_worker = Some(rx);
                tracing::info!("loading policy reload candidate while disabled");
            }
        }
        ...
        let (mut targets, action, gain, moving, policy_label) = match (driving, sensors.as_ref()) {
            ...
            (true, Some(sensors)) => {
                ...
                match controller.step(sensors, &command, snapshot.pose.active, dt, scale_mult) {
                    ...
                    Err(e) => {
                        ...
                        (hold, None, policy_cfg.gain, false, "held")
                    }
                }
            }
            ...
            _ => (hold, None, policy_cfg.gain, false, "held"),
        };
...
/// Coalesce SIGHUP into one request the control loop can handle at a safe tick boundary.
async fn watch_reload_requests(state: Arc<RobotState>) {
    use tokio::signal::unix::{SignalKind, signal};
    let Ok(mut hup) = signal(SignalKind::hangup()) else {
        tracing::warn!("cannot listen for SIGHUP");
        return;
    };
    while hup.recv().await.is_some() {
        state.reload_requested.store(true, Ordering::Relaxed);
        tracing::info!("policy reload requested; waiting until policy is disabled");
    }
}
...
mod tests {
    ...
    #[test]
    fn only_policy_driven_ticks_have_imitation_labels() {
        assert!(recorded_policy_action(None).is_empty());
        assert_eq!(
            recorded_policy_action(Some([0.0; duck_control::ACTION_LEN])),
            vec![0.0; duck_control::ACTION_LEN]
        );
    }

    #[test]
    fn a_reload_is_deferred_until_policy_control_is_off_and_the_worker_is_free() {
        assert!(
            !may_start_reload(true, false, true),
            "never reload while driving"
        );
        assert!(!may_start_reload(false, true, true), "one worker at a time");
        assert!(
            !may_start_reload(false, false, false),
            "signals are requests, not polls"
        );
        assert!(may_start_reload(false, false, true));
    }
