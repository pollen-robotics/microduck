...
| everything on the wire should be timestamped at source | `remote-webrtc.md` §11. **The control half is built** (v24: `t_ns` on `robot.state`/`tof.frame`/`head_imu.frame`, plus `media.video`'s `mono_ns`/`real_ns` pair that puts them on the wall clock; additive, so no bump). **The media half was checked and the extension is a dead end**: the member exists on paper only and nothing on the wire fills it. Frames are dated from RTCP sender reports instead — which works only for a receiver that reads the report's **RTP** time as well as its NTP time, i.e. GStreamer (`sr-rtptime`); `aiortc` and every browser see the NTP half alone |
